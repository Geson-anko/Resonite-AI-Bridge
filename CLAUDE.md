...
- **トランスポート**: 本番 gRPC IPC の `$XDG_RUNTIME_DIR/resonite-io/` とは分離した `$XDG_RUNTIME_DIR/resonite-io-debug/host-agent.sock` を使う。mod 側 socket 探索ロジック (`resonite-*.sock`) と命名衝突しない設計。
