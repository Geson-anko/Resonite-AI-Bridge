...
- `just check-gale` が exit 0 を返す状態 (BepisLoader / BepInEx.Core / BepInExResoniteShim / BepisResoniteWrapper が in)
...
### `BepisResoniteWrapper` 不在

ログに `Could not load type` / `FileNotFoundException` などで `BepisResoniteWrapper` (Harmony ラッパ) を解決できないエラーが出る場合、Gale で `ResoniteModding-BepisResoniteWrapper` (>=1.0.2) を install していない。Gale で追加して `just check-gale` を再走。
